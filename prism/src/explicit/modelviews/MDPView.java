 ...
 */
public abstract class MDPView extends ModelView<Double> implements MDP<Double>, Cloneable
{
	public MDPView()
	{
		super();
	...
	public String toString()
	{
		...
		for (int state = 0, numStates = getNumStates(); state < numStates; state++) {
			...
			for (int choice = 0, numChoices = getNumChoices(state); choice < numChoices; choice++) {
				...
				if (action != null)
					s += action + ":";
				s += new Distribution<>(getTransitionsIterator(state, choice), evaluator);
			}
			s += "]";
		}
		s += " ]";
	...
	//TODO LuPa: Compare with default implementation
	@Override
	...
	@Override
	public void mvMultMinMax(final double[] vect, final boolean min, final double[] result, final PrimitiveIterator.OfInt subset, final int[] strat)
	{
		for (FunctionalPrimitiveIterator.OfInt states = (FunctionalPrimitiveIterator.OfInt) subset; states.hasNext();) {
			final int state = states.nextInt();
			result[state] = mvMultMinMaxSingle(state, vect, min, strat);
		}
	}
	...
	@Override
	public double mvMultGSMinMax(final double[] vect, final boolean min, final PrimitiveIterator.OfInt subset, final boolean absolute,
			final int[] strat)
	{
		double maxDiff = 0.0;
	...
	@Override
	public void mvMultRewMinMax(final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final double[] result, final BitSet subset,
	...
	@Override
	public void mvMultRewMinMax(final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final double[] result, final PrimitiveIterator.OfInt subset,
			final int[] strat)
	{
		for (FunctionalPrimitiveIterator.OfInt states = (FunctionalPrimitiveIterator.OfInt) subset; states.hasNext();) {
			final int state = states.nextInt();
	...
	@Override
	public double mvMultRewMinMaxSingle(final int state, final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final int[] strat)
	{
		int stratCh = -1;
		double minmax = 0;
		boolean first = true;
	...
	@Override
	public double mvMultRewSingle(final int state, final int choice, final double[] vect, final MCRewards<Double> mcRewards)
	{
		// Compute sum for this distribution
	...
	@Override
	public double mvMultRewGSMinMax(final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final BitSet subset, final boolean complement,
	...
	@Override
	public Model<Double> constructInducedModel(final MDStrategy<Double> strat)
	{
		return new DTMCFromMDPAndMDStrategy<>(this, strat);
	}

	@Override
	public double mvMultRewGSMinMax(final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final PrimitiveIterator.OfInt subset,
			final boolean absolute, final int[] strat)
	{
		double maxDiff = 0.0;
	...
	@Override
	public double mvMultRewJacMinMaxSingle(final int state, final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final int[] strat)
	{
		int stratCh = -1;
		double minmax = 0;
		boolean first = true;
	...
	@Override
	public List<Integer> mvMultRewMinMaxSingleChoices(final int state, final double[] vect, final MDPRewards<Double> mdpRewards, final boolean min, final double val)
	{
		// Create data structures to store strategy
		final List<Integer> result = new ArrayList<Integer>();
