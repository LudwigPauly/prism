...
public class CTMCRestricted<Value> extends CTMCView<Value>
{
	...
	public static<Value> BasicModelTransformation<CTMC<Value>, CTMCRestricted<Value>> transform(final CTMC<Value> model, final IntPredicate states)
	{
		return transform(model, Reducible.unboxInt(new IterableStateSet(states, model.getNumStates())).collect(new BitSet()));
	}

	public static <Value> BasicModelTransformation<CTMC<Value>, CTMCRestricted<Value>> transform(final CTMC <Value> model, final BitSet states)
	{
		return transform(model, states, STANDARD_RESTRICTION);
	}

	public static <Value> BasicModelTransformation<CTMC<Value>, CTMCRestricted<Value>> transform(final CTMC<Value> model, final BitSet states, final Restriction restriction)
	{
