...
public class UnionSet<T> extends AbstractSet<T>
{
	...
	{
		FunctionalIterator<T> iterable1 = Reducible.extend(set1).iterator();
		FunctionalIterator<T> iterable2 = Reducible.extend(set2).filter(notInSet1).iterator();
	...
	public int size()
	{
		if (size < 0) {
			int size1 = set1.size();
			int size2 = (int) Reducible.extend(set2).filter(notInSet1).count();
			size = size1 + size2;
		}
		return size;
	}
