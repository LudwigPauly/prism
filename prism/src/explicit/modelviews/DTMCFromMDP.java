...
public class DTMCFromMDP<Value> extends DTMCView<Value>
{
	protected MDP model;
	...
	@Override
	public Iterator<Entry<Integer, Pair<Value, Object>>> getTransitionsAndActionsIterator(int state)
	{
		if (model.getNumChoices() == 0) {
			return Collections.emptyIterator();			
		}
		Object action = model.getAction(state, 0);
		Iterator<Entry<Integer, Value>> transitions = getTransitionsIterator(state);
		return new MappingIterator.ObjToObj<>(transitions, transition -> DTMC.attachAction(transition, action));
	}
