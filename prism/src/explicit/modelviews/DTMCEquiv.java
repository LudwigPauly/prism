...
public class DTMCEquiv extends DTMCView
{
	...
	protected DTMC <Double> model;
	protected EquivalenceRelationInteger identify;
	protected boolean normalize;
	protected BitSet hasTransitionToNonRepresentative;

	public DTMCEquiv(final DTMC<Double> model, final EquivalenceRelationInteger identify, boolean normalize)
	{
		this.model = model;
	...
	public Iterator<Entry<Integer, Double>> getTransitionsIterator(int state)
	{
		...
		if (eqClass == null) {
			...
			return new Distribution(transitions,getEvaluator()).iterator();
		}
		// Dedupe using Distribution
		// Do not use stream>>flatMap, since it preallocates memory unnecessarily
		Distribution<Double> distribution = new Distribution<>(new IterableBitSet(eqClass).iterator().flatMap((int s) -> mapTransitionsToRepresentative(s)),getEvaluator());
		if (normalize) {
	...
	public static BasicModelTransformation<DTMC<Double>, ? extends DTMC<Double>> transform(DTMC<Double> model, EquivalenceRelationInteger identify)
	{
		return transform(model, identify, true);
	}

	public static BasicModelTransformation<DTMC<Double>, ? extends DTMC<Double>> transform(DTMC<Double> model, EquivalenceRelationInteger identify, boolean normalize)
	{
		return transform(model, identify, normalize, false);
	}

	public static BasicModelTransformation<DTMC<Double>, ? extends DTMC<Double>> transform(DTMC<Double> model, EquivalenceRelationInteger identify, boolean normalize, boolean removeNonRepresentatives)
	{
		BasicModelTransformation<DTMC<Double>, DTMCEquiv> quotient = new BasicModelTransformation<>(model, new DTMCEquiv(model, identify, normalize));
		if (! removeNonRepresentatives) {
			return quotient;
		}
		BitSet representatives = identify.getRepresentatives(model.getNumStates());
		BasicModelTransformation<DTMC<Double>, DTMCRestricted> restriction = DTMCRestricted.transform(quotient.getTransformedModel(), representatives, Restriction.TRANSITIVE_CLOSURE_SAFE);
