...
public class MDPModelviewsTest {
    ...
    @Test
    public void additionalChoicesNoAction() throws PrismException {
        init();
        ...
        MDP alteredChoices = new MDPAdditionalChoices(original1, choices, null);
        alteredChoices.findDeadlocks(false);

        assertEquals(exportDotFileToString(original1), exportDotFileToString(alteredChoices));
    ...
    public void equiv() throws PrismException {
        ...
        eq = new EquivalenceRelationInteger(new IterableArray.Of<>(BitSetTools.asBitSet(0, 1, 2)));
        equiv = new MDPEquiv(original1, eq);
        equiv.findDeadlocks(true);


        equiv2 = new MDPSimple(3);
        equiv2.addInitialState(0);
        dist = new Distribution();
        dist.add(0, 1.0);
        equiv2.addActionLabelledChoice(0, dist, "a");
        ...
        dist = new Distribution();
        dist.add(1, 1.0);
        equiv2.addChoice(1, dist);
        dist = new Distribution();
        dist.add(2, 1.0);
        equiv2.addChoice(2, dist);

        // TODO Lupa: look at state 0
        assertEquals(exportDotFileToString(equiv2), exportDotFileToString(equiv));
    }
