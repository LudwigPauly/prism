...
/**
 * class for solving system of linear equations with gauss elimination for functions
 */
public class GaussElimination
{
	...
	public Function[] solve(Function[][] a, Function[] b, FunctionFactory factory) throws PrismException
	{
		...
		for (int i = 0; i < n - 1; i++) {
			...
			for (int k = i + 1; k < n; k++) {
				// check for zero
				if (a[i][i].isZero()) {
					throw new PrismException("Cannot divide by Zero");
				}
				//compute l
				//l[k][i] = u[k][i] - u[i][i]
	...
	public void decomposition(Function[][] a, FunctionFactory factory) throws PrismException
	{
		if (a.length != a[0].length)
			throw new PrismException("Matrix A must be a square matrix!");
	...
	/**
	 * lu decomposition according to gauss elimination
	 * @throws PrismException when divided by zero
	 */
	...
	/**
	 * forward substitution according to gauss elimination
	 * @throws PrismException when divided by zero
	 */
	protected void forwardSubstitution() throws PrismException
	{
		Function tmp;
	...
	/**
	 * backwards substitution according to gauss elimination
	 * @throws PrismException when divided by zero
	 */
	protected void backwardSubstitution() throws PrismException
	{
		Function tmp;
