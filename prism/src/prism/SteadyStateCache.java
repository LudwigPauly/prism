...
import prism.SteadyStateProbs.SteadyStateProbsParam;
...
public class SteadyStateCache implements PrismSettingsListener
{
	...
	public SteadyStateProbsParam getSteadyStateProbs(ParamModel model)
	{
		if (!containsSteadyStateProbs(model)) {
			throw new NoSuchElementException("No steady-state probabilities stored for model.");
		}

		return (SteadyStateProbsParam) probabilities;
	}
	...
	public SteadyStateProbsParam storeSteadyStateProbs(ParamModel model, SteadyStateProbsParam probs, PrismSettings settings)
	{
		return store(model, probs, settings);
	}
